...
public class MainActivity extends AppCompatActivity
    ...
    private static final int MOVE_DETECTION_INTERVAL = 6000;
    private boolean cameraOnLogged = false;
    // [추가] lee
    private Handler missingAlertHandler = new Handler(Looper.getMainLooper());
    //private AlertDialog missingDialog = null;.
        ...
        warnRunnable = new Runnable() {
            ...
            public void run() {
                if(System.currentTimeMillis() - airWarnTime > MOVE_DETECTION_INTERVAL){
                    ...
                    warnHandler.postDelayed(this,50000);
                }
            }
        };
...
//    }
    ...
    // [이 함수가 딱 하나만 존재해야 합니다!]
// [수정] analyze 함수: 화면 관련 코드 제거 버전
    private void analyze(ImageProxy imageProxy) {
        if (imageProxy.getImage() == null) {
            imageProxy.close();
            return;
                            ...
                            if (isWarningActive && moveDetection.isPersonVisible()) {
                                runOnUiThread(() -> {
                                    if (isWarningActive) {
                                        ...
                                        // 안심 멘트
                                        robot.speak(TtsRequest.create("사용자가 인식되었습니다. 알림을 취소합니다.", false));
                                    }
                                });
                            }
    ...
    // [수정] 사라짐 경고 다이얼로그 및 10초 타이머 로직 ,, lee_first
// [최종] 화면 없이 '소리'로만 경고하는 함수
